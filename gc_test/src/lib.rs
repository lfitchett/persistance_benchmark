...
impl Storage for GCStorage {
    fn write(&mut self, session_id: &str, publishes: &[Publish]) -> Result<(), Box<dyn Error>> {
        let messages = publishes.iter().map(|publish| {
            self.write_payload_if_empty(&publish.payload).unwrap();
        ...
        self.write_body(session_id, messages)?;
        Ok(())
    }

    fn read(&mut self, session_id: &str) -> Result<Vec<Publish>, Box<dyn Error>> {
        let bodies = self.bodies.join(session_id);
        if !bodies.exists() {
            return Ok(Vec::new());
        }

        let result: Vec<DiskPublish> = bincode::deserialize_from(File::open(bodies)?)?;
        let result: Vec<Publish> = result
            .iter()
            .map(|body| {
                let payload = self.get_payload(body.payload_id).unwrap();
...
impl GCStorage {
    pub fn new(location: &Path) -> Result<Self, Box<dyn Error>> {
        ...
        let bodies = location.join("Sessions");
    ...
    fn write_body<B>(&self, session_id: &str, bodies: B) -> Result<(), Box<dyn Error>>
    where
        B: Iterator<Item = DiskPublish>,
    {
        let dir = self.bodies.join(session_id);

        let bodies: Vec<DiskPublish> = bodies.collect();
        let bytes = bincode::serialize(&bodies)?;
    ...
    fn get_session_ids(&self) -> Result<Vec<String>, Box<dyn Error>> {
        Self::list_children(&self.bodies)
    }

    fn get_session_payload_ids(&self, session_id: &str) -> Result<Vec<u64>, Box<dyn Error>> {
        let path = self.bodies.join(session_id).join("Messages");
